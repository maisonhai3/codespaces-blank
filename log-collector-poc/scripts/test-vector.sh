...
# Test script for Vector collector

source "$(dirname "$0")/common.sh"

TOOL_NAME="vector"
COLLECTOR_CONTAINER="log-collector-poc-vector-1"
RESULTS_FILE="$RESULTS_DIR/vector-results.md"

main() {
    log_info "Starting Vector PoC tests..."
    
    generate_report_header "Vector" "$RESULTS_FILE"
    ...
    stop_services
    log_success "Vector tests completed! Results saved to: $RESULTS_FILE"
}

test_baseline_performance() {
    local test_duration=60
    local stats_file="$RESULTS_DIR/vector-baseline-stats.csv"
...
test_offline_resilience() {
    ...
    local stats_file="$RESULTS_DIR/vector-offline-stats.csv"
