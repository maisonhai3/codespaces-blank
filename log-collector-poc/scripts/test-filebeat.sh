...
# Test script for Filebeat collector

source "$(dirname "$0")/common.sh"

TOOL_NAME="filebeat"
COLLECTOR_CONTAINER="log-collector-poc-filebeat-1"
RESULTS_FILE="$RESULTS_DIR/filebeat-results.md"

main() {
    log_info "Starting Filebeat PoC tests..."
    
    generate_report_header "Filebeat" "$RESULTS_FILE"
    ...
    stop_services
    log_success "Filebeat tests completed! Results saved to: $RESULTS_FILE"
}
...
test_baseline_performance() {
    ...
    local analysis=$(analyze_stats "$stats_file" "filebeat")
...
test_backpressure_handling() {
    ...
    local stats_file="$RESULTS_DIR/filebeat-backpressure-stats.csv"
